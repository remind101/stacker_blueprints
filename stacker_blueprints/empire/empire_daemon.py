...
class EmpireDaemon(Blueprint):
    PARAMETERS = {
        ...
        'GitHubDeploymentsEnvironment': {
            'type': 'String',
            'description': 'Environment used for GitHub Deployments and honeybadger',
            'default': ''},
        'TokenSecret': {
            'type': 'String',
        ...
        'PrivateAppELBSG': {
            'type': 'String',
            'description': 'Security group to attach to internal load balancers',
            'default': ''},
        'PublicAppELBSG': {
            'type': 'String',
    ...
    def create_custom_cloudformation_resources(self):
        ...
        topic = sns.Topic(
            ...
            Subscription=[
                sns.Subscription(Protocol="sqs", Endpoint=GetAtt("CustomResourcesQueue", "Arn"))])
    ...
    def get_empire_environment(self):
        ...
        return [
            ecs.Environment(Name='EMPIRE_ENVIRONMENT', Value=Ref('Environment')),
            ecs.Environment(Name='EMPIRE_SCHEDULER', Value='cloudformation-migration'),
            ecs.Environment(Name='EMPIRE_REPORTER', Value=Ref('Reporter')),
            ecs.Environment(Name='EMPIRE_S3_TEMPLATE_BUCKET', Value=Ref('TemplateBucket')),
            ecs.Environment(Name='EMPIRE_GITHUB_CLIENT_ID', Value=Ref('GitHubClientId')),
            ecs.Environment(Name='EMPIRE_GITHUB_CLIENT_SECRET', Value=Ref('GitHubClientSecret')),
            ecs.Environment(Name='EMPIRE_DATABASE_URL', Value=database_url),
            ecs.Environment(Name='EMPIRE_TOKEN_SECRET', Value=Ref('TokenSecret')),
            ecs.Environment(Name='AWS_REGION', Value=Ref('AWS::Region')),
            ecs.Environment(Name='EMPIRE_PORT', Value='8081'),
            ecs.Environment(Name='EMPIRE_GITHUB_ORGANIZATION', Value=Ref('GitHubOrganization')),
            ecs.Environment(Name='EMPIRE_GITHUB_WEBHOOKS_SECRET', Value=Ref('GitHubWebhooksSecret')),
            ecs.Environment(Name='EMPIRE_GITHUB_DEPLOYMENTS_ENVIRONMENT', Value=Ref('GitHubDeploymentsEnvironment')),
            ecs.Environment(Name='EMPIRE_EVENTS_BACKEND', Value='sns'),
            ecs.Environment(Name='EMPIRE_SNS_TOPIC', Value=Ref('EventsTopic')),
            ecs.Environment(Name='EMPIRE_TUGBOAT_URL', Value=Ref('TugboatUrl')),
            ecs.Environment(Name='EMPIRE_LOGS_STREAMER', Value=Ref('LogsStreamer')),
            ecs.Environment(Name='EMPIRE_ECS_CLUSTER', Value=Ref('MinionCluster')),
            ecs.Environment(Name='EMPIRE_ECS_SERVICE_ROLE', Value='ecsServiceRole'),
            ecs.Environment(Name='EMPIRE_ROUTE53_INTERNAL_ZONE_ID', Value=Ref('InternalZoneId')),
            ecs.Environment(Name='EMPIRE_EC2_SUBNETS_PRIVATE', Value=Join(',', Ref('PrivateSubnets'))),
            ecs.Environment(Name='EMPIRE_EC2_SUBNETS_PUBLIC', Value=Join(',', Ref('PublicSubnets'))),
            ecs.Environment(Name='EMPIRE_ELB_SG_PRIVATE', Value=Ref('PrivateAppELBSG')),
            ecs.Environment(Name='EMPIRE_ELB_SG_PUBLIC', Value=Ref('PublicAppELBSG')),
            ecs.Environment(Name='EMPIRE_GITHUB_DEPLOYMENTS_IMAGE_BUILDER', Value='conveyor'),
            ecs.Environment(Name='EMPIRE_CONVEYOR_URL', Value=Ref('ConveyorUrl')),
            ecs.Environment(Name='EMPIRE_RUN_LOGS_BACKEND', Value='cloudwatch'),
            ecs.Environment(Name='EMPIRE_CUSTOM_RESOURCES_TOPIC', Value=Ref('CustomResourcesTopic')),
            ecs.Environment(Name='EMPIRE_CUSTOM_RESOURCES_QUEUE', Value=Ref('CustomResourcesQueue')),
            ecs.Environment(Name='EMPIRE_CLOUDWATCH_LOG_GROUP', Value=Ref('RunLogs'))
        ]
    ...
    def create_ecs_resources(self):
        ...
        t.add_resource(
            ecs.TaskDefinition(
                'TaskDefinition',
                Volumes=[
                    ecs.Volume(Name='dockerSocket', Host=ecs.Host(SourcePath='/var/run/docker.sock')),
                    ecs.Volume(Name='dockerCfg', Host=ecs.Host(SourcePath='/root/.dockercfg'))],
                ContainerDefinitions=[
                    ecs.ContainerDefinition(
                        ...
                        MountPoints=[
                            ecs.MountPoint(SourceVolume='dockerSocket', ContainerPath='/var/run/docker.sock', ReadOnly=False),
                            ecs.MountPoint(SourceVolume='dockerCfg', ContainerPath='/root/.dockercfg', ReadOnly=False)],
                        PortMappings=[
